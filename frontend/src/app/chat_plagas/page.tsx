...
import { fetchFromPlagasAPI } from "@/services/ia-service";
...
export default function Chat() {
  ...
  const handleChatPlagas = async () => {
    ...
    const historial = getChatHistoryAsText();

    setIsWaitingResponse(true);
    try {
      const response = await fetchFromPlagasAPI(actualMessage, historial, controller.signal);
      ...
      console.log("Respuesta recibida:", cleanMarkdownResponse(response.response));
    } catch (error: unknown) {
  ...
  const getChatHistoryAsText = (): string => {
    return chatHistory
      .map((msg) => {
        const sender = msg.sender === "user" ? "Usuario" : "IA";
  ...
  const handleMicClick = async () => {
    if (!isRecording) {
      ...
      const result = await audioServiceRef.current?.sendVoiceStreamToPlagas(audioBlob!);
      ...
      setChatHistory((prev) => [
        ...prev,
        { sender: "user", text: cleanMarkdownResponse(result.transcription) },
      ]);

      setIsWaitingResponse(true);
      const response = await fetchFromPlagasAPI(result.transcription, getChatHistoryAsText());
      ...
      setChatHistory((prev) => [
        ...
        {
          sender: "ia",
          text: cleanMarkdownResponse(response.response || "No se recibió respuesta"),
        },
      ]);
    }
  };
